...
function stylesbuild() {
    return src(siteconfig.build.links.source.assets.scss + '/**/*.scss')
        ...
        .pipe(gulpif(__release, autoprefixer({ overrideBrowserslist: ['last 10 versions'], grid: true, cascade: false })))
        .pipe(gulpif(__release, cleancss({ level: { 1: { specialComments: 0 } } })))
        .pipe(gulpif(__release, cssnano()))
        .pipe(gulpif(!__release, sourcemaps.write()))
        .pipe(dest(siteconfig.build.links.build.assets.css + '/'))
        .pipe(gulpif(!__release, browserSync.stream()));
}
...
function pg() {
    return src([siteconfig.build.links.source.pages + '/**/*.pug'])
        ...
        .pipe(plumber.stop())
        .pipe(rep('sender.php', filenameCashe))
...
exports['build:release'] = series(cleanbuild, jsonldeditor, jsoncontent, buildcopy, buildres, buildsender, pg, scripts, icons, stylesbuild, askAllImages, replaceAllImagesHTML, replaceAllImagesCSS, replaceAllImagesJS, createsitemap, createrobots);
exports['build:dev'] = series(cleanbuild, jsoncontent, parallel(buildcopy, buildres, scripts), icons, buildsender, pg, stylesbuild);
exports.default = series(series(cleanbuild, jsoncontent, parallel(buildcopy, buildres, scripts), icons, buildsender, pg, stylesbuild), serve, browsersync);
